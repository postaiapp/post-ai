import { Button } from "@components/ui/button";
import { Input } from "@components/ui/input";
import Image from "next/image";

export default function Home() {
  return (
    <div className="flex items-center justify-center h-screen w-full">
      <div className="flex flex-col items-center justify-center h-[80%] w-[65%]">
        <div>
          <div className="p-3 mx-auto rounded-md border-[1.5px] border-gray-200 w-fit mb-4 shadow-sm">
            <Image src="/logo.png" alt="Logo" width={48} height={48} />
          </div>
          <p className="text-2xl text-center font-semibold bg-gradient-to-r from-purple-500 to-purple-400 text-transparent bg-clip-text mb-2">
            Gere posts com o POST-AI
          </p>
          <p className="text-sm font-thin text-center mx-auto text-gray-400 w-80">
            Escolha seu prompt abaixo ou escreva seu próprio texto para gerar um
            post incrível!
          </p>

          <div className="grid grid-cols-3 gap-4 w-full mt-8">
            {Array.from({ length: 6 }).map((_, index) => (
              <Button key={index} className="rounded-2xl" variant="outline">
                Prompt {index + 1}
              </Button>
            ))}
          </div>
        </div>

        <div className="mt-auto">
          <Input className="w-96" placeholder="Escreva seu texto aqui..." />
        </div>
      </div>
    </div>
  );
}
