...
export class MetaStrategy {
	private readonly metaStrategyLogger = new Logger(MetaStrategy.name);
	...
	async exchangeCodeForToken(code: string): Promise<{
		...
		try {
			...
			this.metaStrategyLogger.error('Error exchanging code for token:', error);
			throw error;
		}
	}
	...
	async connect(data: CreatePlatformDto, userId: number) {
		this.metaStrategyLogger.log('Connecting to Instagram with data:', data);
	...
	private async getInstagramProfile(userId: string, accessToken: string) {
		const response = await InstagramClient({
			...
			url: `/${userId}`,
			params: {
				fields: 'username,name,profile_picture_url,media_count,followers_count,follows_count,biography,website,id',
				access_token: accessToken,
			},
			headers: {
