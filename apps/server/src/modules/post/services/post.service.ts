...
export class PostService {
	...
	async schedulePost({ body, meta }: PostCreate) {
		...
		const hasUserAccount = await this.instagramAuthService.hasInstagramAccount(meta, username);

		if (!hasUserAccount) {
			throw new NotFoundException('User dont have this account');
		}
		...
		post = await this.createPostRecord({
			caption,
		...
		const job = new CronJob(
			...
			async () => {
				...
				try {
					await this.publishPost({ body, meta, id: post._id.toHexString() });
				} catch (err) {
	...
	async publishPost({ body, meta, id }: PublishedPostProps) {
		...
		const newPost = await this.postModel.findOneAndReplace(
			{ _id: id },
			{
