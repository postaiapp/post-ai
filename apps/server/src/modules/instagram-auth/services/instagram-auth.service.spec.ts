...
describe('InstagramAuthService', () => {
	...
	const mockStorageService = {
		...
		downloadAndUploadImage: jest
			.fn()
			.mockImplementation(url => Promise.resolve({ url: 'uploaded-url' })),
	};
	...
	describe('hasInstagramAccount', () => {
		it('should throw NotFoundException if user not found', async () => {
			...
			await expect(service.hasInstagramAccount(mockMeta, 'testuser')).rejects.toThrow(
				NotFoundException,
			);
	...
	describe('getAccounts', () => {
		it('should return user accounts', async () => {
			...
			mockStorageService.getSignedImageUrl = jest
				.fn()
	...
	describe('getToken', () => {
		it('should return a valid session token', async () => {
			...
			const expectedState = Buffer.from(JSON.stringify(mockSerializedState)).toString(
				'base64',
			);
